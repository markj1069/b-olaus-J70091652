...
echo "Arg: $*"
    ...
    if [[ -z "$1" ]]; then
        ols_err $EX_USAGE 7001 "assert: missing argument 1, condition"
    else
        local CONDITION="$1"
    ...
    if [[ -z "$2" ]]; then
        ols_err $EX_USAGE 7002 "assert: missing argument 2, lineno"
    else
        local LINENO="$2"
    fi
    ...
    if [[ ! $CONDITION ]]
    then
        ols_err 7002 $EX_ASSERTFAIL "assert: Assertion Fail. '$CONDITION', File '$0', line '$LINENO'"
    fi
